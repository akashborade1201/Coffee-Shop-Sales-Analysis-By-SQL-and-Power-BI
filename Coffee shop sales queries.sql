...
-- Chnahing text date to date format
...
-- Changing datatype  text to time 
...
select month(transaction_date) as motnhs,
...
where month(transaction_date) in (4,5) -- for april and may month
...
-- average per day for particular month

select day(transaction_date) as monthday,
sum(transaction_qty * unit_price) as total_sales,
avg(sum(transaction_qty * unit_price)) over() as avge
from coffee_shop_sales
